import profile from '../images/f06bea95d2c425676a1a531f0c7159fe.jpg';
    ...
    >
      <div className='flex justify-between w-40 mt-3 lg:mt-0 lg:flex-col lg:w-auto lg:h-40'>
        {[
          ...
          ['fa-linkedin', 'https://www.linkedin.com/in/rohan-shrestha-9b5580232/'],
        ].map((element, index) => (
      ...
      <div className='text-center lg:text-left h-60 flex flex-col justify-between items-center lg:items-start md:w-3/4 lg:w-1/2'>
        ...
        <div className='text-3xl font-bold lg:text-5xl flex items-center'>
          {str.map((element, index) => (
            <p key={index} className='hover:text-rose-500 transition-all cursor-default'>
              {element === ' ' ? '\u00a0' : element}
            </p>
          ))}
        </div>
        ...
        <p className='text-sm lg:text-base'>
          I'm a young developer currently studying at Bachelor level. I am available for projects
          and internship.
        </p>
        <a
