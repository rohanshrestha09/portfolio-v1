...
const Nav = () => {
  ...
  return (
    <nav className="z-10 flex w-full justify-between items-center fixed left-0 top-0 h-[4rem] xl:px-44 lg:px-20 px-7">
      <p className="font-stylish font-thin text-xl text-rose-500 md:text-2xl">
        RohanShrestha
      </p>
      <i
      ...
      <div
        ...
        } flex-col justify-evenly h-44 md:h-auto md:flex md:static md:flex-row md:w-96 md:justify-between text-right`}
      >
        {["About", "Skills", "Portfolio", "Contact"].map((element, index) => (
          <a
            href={`${
              element === "Portfolio" ? "#skills" : `#${element.toLowerCase()}`
            }`}
            key={index}
